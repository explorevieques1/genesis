...
Design a multi-artboard concept canvas for **Genesis**, an always-on agentic trading
terminal. Genesis is a JARVIS-style command layer: the trader commands it — by typing or by
speaking — and an orchestrator acts as their **senior market analyst**, putting a fleet of
~30 specialist agents (research, chart markup, strategy/backtest, execution/risk,
journal/insight) to work over a task bus, behind an unbypassable pre-trade risk gate.

**The framing to design for is a firm, not an app.** The user is the head trader. Genesis is
the analyst who reports to them. The agents are the staff. The feeling to produce is *"I have
thirty analysts, quants and researchers on my desk"* — hours of research collapsing into a
briefing — not *"I have a chatbot with a dashboard attached."*
...
Three rules bind the design and are not negotiable:

- **The human commands; Genesis advises.** Genesis can open tools and arrange the screen on
  its own — but every single thing it can do, the operator can also do by hand, from the same
  command line. Nothing lives behind the voice.
- **The canvas opens empty.** No landing dashboard, no unrequested telemetry. Panels exist
  because someone asked.
- **Data is never merely presented.** Bloomberg presents. Genesis *collects, computes and
  orchestrates* — every panel carries the agent that produced it, what it read, and as of
  when.

---

## 1. The landing screen: an empty canvas, and the core

The landing screen holds **a command line, a thin safety strip, and the Genesis core.
Nothing else.** No positions table, no market overview, no capability report — an empty
canvas the operator fills by asking for something.

The core is a large **3D vector animation of "Genesis"** — a living wireframe intelligence,
centered, on near-black — but it is presence, not content. The **command line is the focal
point of the composition**, and it is the map of the whole system: typing one character
reveals what can be asked for (pages, panels, symbols, agents, every tool by name). Show that
reveal as its own artboard — the empty state, and the same screen one keystroke later.

An empty screen with a bare cursor is a dead end; solve it in the *input*, not by putting
panels back. A row of literal example commands under the field — *"why is Nvidia down
today"*, *"backtest my last idea"*, *"research Gann and save it to my journal"* — that
vanishes on the first character is the intended shape.
...
The input bar is **one field, two ways in**: a text command line with a push-to-talk button
and level meter beside it. Not a text "fallback" — typed and spoken reach the same place, and
a design that makes the microphone look like the real input is describing the wrong product.
Include the wake-word indicator and the live-transcript line.

**Show name resolution in the input.** The trader types *"nvidia"*; the field resolves it to
a chip reading `NVDA — NVIDIA Corp` before anything runs, and an ambiguous name shows a
disambiguation list rather than picking. Nobody knows the ticker; the system must never
require one.
...
- Every panel has: title, live/stale data indicator, **the agent that produced it**, timestamp,
  **the request that opened it**, and dismiss/pin controls. Pinned panels survive the next
  question. A panel that appeared with no stated cause is indistinguishable from a bug — the
  chassis must make "why am I looking at this?" answerable without asking.
- Panels the operator opened by hand and panels Genesis opened are the **same chassis**,
  distinguished only by that provenance line. There is no separate "agent output" surface.
...
7. **The briefing** — the answer to a question, as an analyst would deliver it: the
   conclusion in one line at the top, then the evidence that produced it — each news item,
   filing, chart and computed figure attributed to the agent that fetched it and stamped with
   as-of. A "show me" on every piece opens the panel it came from. This is the screen that
   distinguishes Genesis from a terminal, so design it before the pretty ones.
8. **Work in flight** — a long research task running: the steps Genesis said it would take,
   which agents are on which, what has landed, what is pending, and where the finished
   artifact will be saved. A multi-minute job must never look like a hang.
9. **Journal & memory** — timeline of trades, decisions, and lessons; linked-note graph view
   of the memory fabric; searchable episodic log.
10. **Alerts / notification rail** — a right-edge stack of agent-pushed cards: setup found,
...
Also include: a **mobile/companion view** (core + voice bar + alert stack only), and the
**empty landing state** from §1 — which is not an edge case here, it is the front door.
...
A canvas of artboards covering: the empty canvas and its one-keystroke reveal, the four core
states, the panel-summoning transition, each of the ten screen types, the split and 6-up layouts, the mobile companion, and a
